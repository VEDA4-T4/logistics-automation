 ...
 * Parser를 초기 상태로 되돌린다.
 */
static void parser_reset_internal(
 ...
 * 새로운 Frame 수신을 시작한다.
 */
static void parser_start_frame(
    uart_parser_t *parser
)
{
 ...
 */
static void parser_reset_and_resync(
    uart_parser_t *parser,
    uint8_t byte
)
{
 ...
 *   0: 버퍼 부족
 */
static uint8_t parser_store_byte(
    uart_parser_t *parser,
    uint8_t byte
)
{
 ...
 */
static uart_parser_result_t map_codec_result(
...
)
{
    ...
    switch (parser->state)
    {
        ...
        case UART_PARSER_READ_CRC_HIGH:
             ...
             */
            codec_result = uart_decode_frame(
                parser->raw_frame,
                parser->raw_length,
                frame
            );
...
)
{
     ...
     * 현재 Frame을 폐기한다.
     */
    if (parser->elapsed_ms >= UART_PARSER_TIMEOUT_MS ||
        elapsed_ms >=
        (UART_PARSER_TIMEOUT_MS - parser->elapsed_ms))
    {
        parser_reset_internal(parser);
