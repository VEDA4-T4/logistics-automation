 ...
 */
uart_codec_result_t uart_encode_frame(
...
)
{
    ...
    *output_length = 0U;
    ...
    /* 전체 Frame 크기 계산 */
    frame_length =
        UART_FRAME_OVERHEAD_SIZE + frame->length;
     ...
     */
    output[5U + frame->length] =
        UART_CRC_LOW_BYTE(crc);

    output[6U + frame->length] =
        UART_CRC_HIGH_BYTE(crc);
 ...
 */
uart_codec_result_t uart_decode_frame(
    const uint8_t *input,
    size_t input_length,
    uart_frame_t *frame
)
{
    ...
    /* 예상되는 전체 Frame 크기 계산 */
    expected_length =
        UART_FRAME_OVERHEAD_SIZE + payload_length;
     ...
     */
    received_crc =
        (uint16_t)input[5U + payload_length];

    received_crc |=
        (uint16_t)input[6U + payload_length] << 8U;

    /*
     * VERSION부터 PAYLOAD까지 CRC 계산
     */
    calculated_crc = uart_crc16_ccitt(
        &input[1],
        4U + payload_length
    );
