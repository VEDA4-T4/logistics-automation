...
build_dir="${LOGISTICS_BUILD_DIR:-${repo_root}/build-linetracer}"
install_prefix="${LOGISTICS_INSTALL_PREFIX:-/opt/logistics-automation}"
runtime_dir="${LOGISTICS_RUNTIME_DIR:-/var/lib/logistics}"
config_path="${LOGISTICS_CONFIG_PATH:-/etc/logistics/linetracer-node.ini}"
service_name="logistics-linetracer-node.service"

central_host="${LOGISTICS_CENTRAL_HOST:-}"
mqtt_host="${LOGISTICS_MQTT_HOST:-${central_host}}"
device_id="${LOGISTICS_DEVICE_ID:-PI-LT-01}"
node_name="${LOGISTICS_NODE_NAME:-linetracer-node-01}"
...
if [[ "$(uname -s)" != "Linux" ]]; then
    echo "This script must run on the linetracer Linux/Raspberry Pi host." >&2
    exit 2
fi
...
if [[ "${write_config}" == "1" &&
      -z "${device_ip}" ]]; then
    echo "Could not detect the linetracer node IP; set LOGISTICS_DEVICE_IP." >&2
    exit 2
fi
...
cmake -S "${repo_root}" -B "${build_dir}" -G Ninja \
    ...
    -DLOGISTICS_BUILD_LINETRACER_NODE=ON \
    -DLOGISTICS_ENABLE_MOSQUITTO_TRANSPORT=ON

cmake --build "${build_dir}" \
    --target logistics_linetracer_node
...
echo
echo "Linetracer node setup complete."
echo "Binary: ${install_prefix}/bin/logistics_linetracer_node"
