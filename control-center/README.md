...
- 메인 화면은 최신 로그 100건으로 시작합니다.
- `http/bearer_token`이 설정되어 있으면 표의 맨 아래에서 계속 아래로 스크롤할 때 중앙서버의 과거 로그를
  cursor 기반으로 100건씩 추가 조회합니다.
- 새 페이지는 표 아래에 이어 붙입니다. `logs/max_buffer_entries` 한도에 도달한 뒤에는 화면 위쪽의 최신 행을
  제거하면서 과거 행을 계속 불러와, 표시 모델과 실시간 상태 버퍼가 같은 메모리 한도를 지킵니다.
