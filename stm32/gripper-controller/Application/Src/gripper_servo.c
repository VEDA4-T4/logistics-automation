...
static int32_t gripper_servo_enable(void* context) {
    ...
    __HAL_TIM_SET_COMPARE(
        &htim3, TIM_CHANNEL_1,
        gripper_servo_angle_to_pulse(GRIPPER_BASE_HOME_ANGLE_DECI_DEG, GRIPPER_BASE_SERVO_MIN_PULSE_US,
                                     GRIPPER_BASE_SERVO_MAX_PULSE_US));
    __HAL_TIM_SET_COMPARE(
        &htim3, TIM_CHANNEL_2,
...
static int32_t gripper_servo_write_arm(void* context, uint16_t base_angle, uint16_t shoulder_angle,
    ...
    __HAL_TIM_SET_COMPARE(
        &htim3, TIM_CHANNEL_1,
        gripper_servo_angle_to_pulse(base_angle, GRIPPER_BASE_SERVO_MIN_PULSE_US,
                                     GRIPPER_BASE_SERVO_MAX_PULSE_US));
