...
sensor_logic_update_t ApplyStableLine(sensor_logic_context_t& context, std::uint8_t left, std::uint8_t right,
                                      std::uint32_t start_ms) {
    sensor_logic_update_t update{};

    for (std::uint32_t sample = 0U; sample < APP_TIMING_LINE_DEBOUNCE_SAMPLES; ++sample) {
        update = UpdateLine(context, left, right, start_ms + (sample * APP_TIMING_SENSOR_PERIOD_MS));
    }
    return update;
}
...
std::uint32_t EmitMarkerStripe(sensor_logic_context_t& context, std::uint32_t start_ms) {
    ...
    return (context.marker_group_last_stripe_at_ms != 0U) ? context.marker_group_last_stripe_at_ms
                                                          : context.snapshot.marker_detected_at_ms;
}
...
void TestLineNormalizationAndDebounce() {
    ...
    auto update = ApplyStableLine(context, 0U, 0U, 0U);
...
#if APP_TIMING_LINE_DEBOUNCE_SAMPLES > 1U
    CHECK_TRUE(UpdateLine(context, 1U, 0U, 50U).event_flags == 0U);
    CHECK_TRUE(UpdateLine(context, 0U, 0U, 60U).event_flags == 0U);
    CHECK_TRUE(context.snapshot.line_state == LINETRACER_LINE_CENTERED);
#endif

    update = ApplyStableLine(context, 1U, 0U, 100U);
    CHECK_TRUE(context.snapshot.line_state == LINETRACER_LINE_LEFT_ONLY);
    CHECK_TRUE((update.event_flags & APP_SENSOR_EVENT_LINE_CHANGED) != 0U);

    update = ApplyStableLine(context, 0U, 1U, 200U);
    CHECK_TRUE(context.snapshot.line_state == LINETRACER_LINE_RIGHT_ONLY);
    CHECK_TRUE((update.event_flags & APP_SENSOR_EVENT_LINE_CHANGED) != 0U);

    update = ApplyStableLine(context, 1U, 1U, 300U);
    CHECK_TRUE(context.snapshot.marker_active != 0U);
    CHECK_TRUE(context.diagnostics.marker_active != 0U);
    CHECK_TRUE(context.snapshot.line_state == LINETRACER_LINE_RIGHT_ONLY);
    CHECK_TRUE((update.event_flags & APP_SENSOR_EVENT_LINE_CHANGED) == 0U);

    update = ApplyStableLine(context, 0U, 0U, 400U);
    CHECK_TRUE(context.snapshot.marker_active == 0U);
    CHECK_TRUE(context.diagnostics.marker_active == 0U);
    CHECK_TRUE(context.snapshot.line_state == LINETRACER_LINE_CENTERED);
    CHECK_TRUE((update.event_flags & APP_SENSOR_EVENT_LINE_CHANGED) != 0U);
...
void TestInvalidMarkerWidthAndNoise() {
    ...
    std::uint32_t marker_event_flags = 0U;
    marker_event_flags |= UpdateLine(context, 0U, 0U, SENSOR_MARKER_MAX_BLACK_MS + 60U).event_flags;
    marker_event_flags |= UpdateLine(context, 0U, 0U, SENSOR_MARKER_MAX_BLACK_MS + 70U).event_flags;
    marker_event_flags |= UpdateLine(context, 0U, 0U, SENSOR_MARKER_MAX_BLACK_MS + 80U).event_flags;
    CHECK_TRUE((marker_event_flags & APP_SENSOR_EVENT_MARKER) != 0U);
...
void TestOverloadAndObstacleHysteresis() {
    ...
    for (std::uint32_t sample = 1U; sample < SENSOR_OBSTACLE_ACTIVATE_SAMPLES; ++sample) {
        SensorLogic_UpdateUltrasonic(&context, 0U, SENSOR_OBSTACLE_ON_MM, 1U, 390U + (sample * 10U), &update);
        CHECK_TRUE(update.event_flags == 0U);
    ...
    SensorLogic_UpdateUltrasonic(&context, 0U, SENSOR_OBSTACLE_ON_MM, 1U,
                                 390U + (SENSOR_OBSTACLE_ACTIVATE_SAMPLES * 10U), &update);
    ...
    for (std::uint32_t sample = 1U; sample < SENSOR_OBSTACLE_CLEAR_SAMPLES; ++sample) {
        update = {};
        SensorLogic_UpdateUltrasonic(&context, 0U, 90U, 1U, 430U + (sample * 10U), &update);
        CHECK_TRUE(update.event_flags == 0U);
        CHECK_TRUE(context.diagnostics.obstacle_mask != 0U);
    }

    update = {};
    SensorLogic_UpdateUltrasonic(&context, 0U, 90U, 1U, 430U + (SENSOR_OBSTACLE_CLEAR_SAMPLES * 10U), &update);
    CHECK_TRUE((update.event_flags & APP_SENSOR_EVENT_OBSTACLE) != 0U);
