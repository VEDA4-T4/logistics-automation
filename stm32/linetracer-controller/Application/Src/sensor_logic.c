...
void SensorLogic_SetUltrasonicMonitoringEnabled(sensor_logic_context_t* context, uint8_t enabled, uint32_t now_ms) {
    uint8_t index;
    uint32_t error_flags;
    ...
    if (context->ultrasonic_monitoring_enabled == enabled) {
        return;
    }

    context->ultrasonic_monitoring_enabled = enabled;
    context->ultrasonic_started_mask = 0U;
    for (index = 0U; index < SENSOR_LOGIC_ULTRASONIC_COUNT; ++index) {
        context->ultrasonic_failure_count[index] = 0U;
        context->ultrasonic_recovery_count[index] = 0U;
        context->ultrasonic_obstacle_on_count[index] = 0U;
        context->ultrasonic_obstacle_off_count[index] = 0U;
        context->ultrasonic_last_success_ms[index] = now_ms;
    }
    ...
    if (enabled == 0U) {
        ...
        context->diagnostics.valid_flags &=
            ~((uint32_t)SENSOR_LOGIC_VALID_ULTRASONIC_FRONT | (uint32_t)SENSOR_LOGIC_VALID_ULTRASONIC_REAR |
              (uint32_t)SENSOR_LOGIC_VALID_ULTRASONIC_LEFT | (uint32_t)SENSOR_LOGIC_VALID_ULTRASONIC_RIGHT);
        error_flags = context->diagnostics.error_flags &
                      ~((uint32_t)SENSOR_LOGIC_ERROR_ULTRASONIC_FRONT | (uint32_t)SENSOR_LOGIC_ERROR_ULTRASONIC_REAR |
...
void SensorLogic_MarkUltrasonicStarted(sensor_logic_context_t* context, uint8_t sensor_index, uint32_t now_ms) {
    ...
    context->ultrasonic_monitoring_enabled = 1U;
...
void SensorLogic_UpdateUltrasonic(sensor_logic_context_t* context, uint8_t sensor_index, uint16_t distance_mm,
    ...
    *distance_field = distance_mm;
    ...
    if ((previous_mask & direction_flag) == 0U) {
        context->ultrasonic_obstacle_off_count[sensor_index] = 0U;
        if (distance_mm <= SENSOR_ULTRASONIC_STOP_DISTANCE_MM) {
        ...
        context->ultrasonic_obstacle_on_count[sensor_index] = 0U;
        if (distance_mm >= SENSOR_ULTRASONIC_CLEAR_DISTANCE_MM) {
...
void SensorLogic_CheckStaleness(sensor_logic_context_t* context, uint32_t now_ms) {
    ...
    if (context->ultrasonic_monitoring_enabled == 0U) {
        SensorLogic_SetErrorFlags(context, error_flags, now_ms);
        return;
    }
